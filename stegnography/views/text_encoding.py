...
from stegnography.forms import TextEncodeForm, TextDecodeForm
from stegano import lsb
from PIL import Image
from django.utils.http import unquote

def text_encode_view(request):
    if request.method == 'POST':
        form = TextEncodeForm(request.POST, request.FILES)
        if form.is_valid():
            # Get the form data
            ...
            return render(request, 'encryption/text/encrypt_result.html', {'stego_image_file_name': file_name_only})

    else:
        form = TextEncodeForm()

    return render(request, 'encryption/text/encode.html', {'form': form})

def text_decode_view(request):
    decoded_message = None
    if request.method == 'POST':
        form = TextDecodeForm(request.POST, request.FILES)
        if form.is_valid():
    ...
    return render(request, 'encryption/text/decode.html', {'form': form, 'decoded_message': decoded_message})
